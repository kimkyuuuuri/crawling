...
while period < 3 and year <3: 
    ...
    try:
        ...
        # 현재 페이지에서 리스트가 필요함.. 반복문을 돌아야할 것 같은데?
        html = driver.page_source
        soup = BeautifulSoup(html, 'html.parser')
        data = soup.find_all('td','tal')
       # <a href="#" onclick="mstView('movie','20137048');return false;" title="국제시장">국제시장</a>

        for i,title in enumerate(data):
            data2 =  title.findAll('a')
            print(data2)
            driver.find_element_by_id(data2).click()
            
        
        
            
        driver.find_element_by_id('<a href="#" onclick="mstView("movie","20137048");return false;" title="국제시장">국제시장</a>').click()
